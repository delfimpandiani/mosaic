...
from mosaic.model.dataset import PerceptualARTstractDataset
...
from mosaic.model.classification import ProjectedPerceptionSimPrediction
...
if __name__ == "__main__":
  ...
  check_for_conf_field(conf["dataset"], args.conf, "perception_json")
  ...
  def collate_fn(batch):
    image, label, label_idx, clusters, perception_nodes = zip(*batch)
    return torch.stack(image), label, torch.stack(label_idx), clusters, perception_nodes
  
  train_data = PerceptualARTstractDataset(Path(conf["dataset"]["train"]), 
  ...
  model = ProjectedPerceptionSimPrediction(
